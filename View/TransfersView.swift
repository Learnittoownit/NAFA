...
struct TransfersView: View {
    ...
    var body: some View {
        // Same pattern as ParentHomeView — blue behind, scroll on top
        ZStack(alignment: .top) {
            // Blue on top, E8EDF2 on bottom — seamless
            VStack(spacing: 0) {
                Color(hex: "2D6DAB")
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 14) {
                        ...
                        // Send Money button — original style
                        ...
                        TransactionHistorySection(expanded: $historyExpanded, transactions: transactions)
                    }
                    .padding(16)
                    .padding(.bottom, 110)
                    // This is the key — fill full remaining height so blue never shows
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 10) {
                    TextField("Enter amount", text: $amount)
                        ...
                        .padding(14).background(Color(hex: "F5F7FA"))
                        .clipShape(RoundedRectangle(cornerRadius: 12))
                        .onChange(of: amount) { _ in showError = false }

                    // Mic — mutually exclusive with note
                    Button {
                        showVoice.toggle(); if showVoice { showNote = false }
                    } label: {
                        ZStack {
                            ...
                            Image(systemName: "mic.fill").font(.system(size: 18))
                                .foregroundColor(showVoice ? .white : Color.nafTextGray)
                        }
                    }

                    // Pencil — mutually exclusive with voice
                    Button {
                        showNote.toggle(); if showNote { showVoice = false }
                    } label: {
                        ZStack {
                            ...
                            Image(systemName: "pencil").font(.system(size: 18))
                                .foregroundColor(showNote ? .white : Color.nafTextGray)
                        }
                    }
                }
            }
            ...
            if showVoice {
                ...
                .background(Color(hex: "1B3A6B")).clipShape(RoundedRectangle(cornerRadius: 14))
                .padding(.horizontal, 20).padding(.top, 10)
                .transition(.opacity.combined(with: .move(edge: .top)))
            }
            ...
            Button {
                ...
                parentVM.sendMoney(to: selectedChildName, amount: amt, type: selectedType.rawValue)
                transactions.insert(TransferRecord(type: selectedType.rawValue, childName: selectedChildName, date: "Today", amount: amt), at: 0)
                isPresented = false
            } label: {
                Text("Send").font(.system(size: 17, weight: .bold)).foregroundColor(.white)
                    .frame(maxWidth: .infinity).frame(height: 54)
                    .background(canSend ? LinearGradient(colors: [Color(hex: "1A3F7A"), Color(hex: "2D6DAB"), Color(hex: "1A3F7A")], startPoint: .leading, endPoint: .trailing) : LinearGradient(colors: [Color.nafTextGray, Color.nafTextGray], startPoint: .leading, endPoint: .trailing))
        ...
        VStack(spacing: 0) {
            ...
            if expanded {
                VStack(spacing: 16) {
                    ...
                    if mode == .repeat {
                        VStack(alignment: .leading, spacing: 10) {
                            Text("PICK A DAY OF THE WEEK").font(.system(size: 10, weight: .bold)).tracking(0.8).foregroundColor(Color.nafTextGray).padding(.horizontal, 14)
                            HStack(spacing: 6) {
                                ForEach(weekDays, id: \.self) { day in
                                    Button { selectedDay = day } label: {
                                        Text(day).font(.system(size: 11, weight: .medium))
                                            ...
                                            .overlay(RoundedRectangle(cornerRadius: 8).stroke(selectedDay == day ? Color.clear : Color.nafLightCard, lineWidth: 1))
                                    }
                                }
                            }.padding(.horizontal, 14)
                        }
                        VStack(alignment: .leading, spacing: 10) {
                            Text("REPEAT EVERY").font(.system(size: 10, weight: .bold)).tracking(0.8).foregroundColor(Color.nafTextGray).padding(.horizontal, 14)
                            HStack(spacing: 10) {
...
struct MultiDateCalendar: View {
    ...
    private var monthTitle: String { let f = DateFormatter(); f.dateFormat = "MMMM yyyy"; return f.string(from: displayDate) }
    private var todayDay: Int { let c = Calendar.current; let t = c.dateComponents([.year,.month], from: Date()); let d = c.dateComponents([.year,.month], from: displayDate); return (t.year == d.year && t.month == d.month) ? c.component(.day, from: Date()) : 0 }
    private var daysInMonth: Int { Calendar.current.range(of: .day, in: .month, for: displayDate)?.count ?? 30 }
    private var firstWeekday: Int { let comps = Calendar.current.dateComponents([.year,.month], from: displayDate); let first = Calendar.current.date(from: comps)!; return Calendar.current.component(.weekday, from: first) - 1 }
    private var grid: [[Int]] {
        var result: [[Int]] = []; var day = 1; var col = firstWeekday; var row = [Int](repeating: 0, count: 7)
        while day <= daysInMonth { row[col] = day; col += 1; if col == 7 { result.append(row); row = [Int](repeating: 0, count: 7); col = 0 }; day += 1 }
        if col > 0 { result.append(row) }; return result
    }
    var body: some View {
        VStack(spacing: 10) {
            HStack {
                Button { displayDate = Calendar.current.date(byAdding: .month, value: -1, to: displayDate) ?? displayDate } label: { Image(systemName: "chevron.left").font(.system(size: 13)).foregroundColor(Color.nafTextGray) }
                Spacer()
                Text(monthTitle).font(.system(size: 14, weight: .semibold)).foregroundColor(Color(hex: "1B3A6B"))
                Spacer()
                Button { displayDate = Calendar.current.date(byAdding: .month, value: 1, to: displayDate) ?? displayDate } label: { Image(systemName: "chevron.right").font(.system(size: 13)).foregroundColor(Color.nafTextGray) }
            }
            HStack(spacing: 0) { ForEach(weekHeaders, id: \.self) { h in Text(h).font(.system(size: 11, weight: .medium)).foregroundColor(Color.nafTextGray).frame(maxWidth: .infinity) } }
            ForEach(0..<grid.count, id: \.self) { r in
                HStack(spacing: 0) {
                    ForEach(0..<7) { c in
                        let day = grid[r][c]
                        if day == 0 { Color.clear.frame(maxWidth: .infinity).frame(height: 34) }
                        else {
                            let isSel = selectedDates.contains(day); let isToday = day == todayDay
                            Button { if isSel { selectedDates.remove(day) } else { selectedDates.insert(day) } } label: {
                                Text("\(day)").font(.system(size: 13, weight: isSel ? .bold : .regular))
                                    ...
                                    .frame(maxWidth: .infinity).frame(height: 34)
                                    .background(isSel ? Color(hex: "2D6DAB") : Color.clear).clipShape(Circle())
...
struct PendingRequestsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("\(pendingCount) PENDING REQUESTS").font(.system(size: 11, weight: .bold)).tracking(0.8).foregroundColor(Color.nafTextGray).padding(.horizontal, 4)
            ForEach(requests.indices, id: \.self) { i in if requests[i].isApproved == nil { PendingRequestCard(request: $requests[i]) } }
        }
...
struct PendingRequestCard: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            HStack(spacing: 10) {
                ZStack { Circle().fill(Color(hex: "EBF0F8")).frame(width: 40, height: 40); Text(request.childInitial).font(.system(size: 16, weight: .bold)).foregroundColor(Color(hex: "2D6DAB")) }
                VStack(alignment: .leading, spacing: 2) {
            ...
            HStack(spacing: 10) {
                Button { withAnimation { request.isApproved = true } } label: { Text("Approve").font(.system(size: 14, weight: .semibold)).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 42).background(Color(hex: "2D7A4F")).clipShape(RoundedRectangle(cornerRadius: 10)) }
                Button { withAnimation { request.isApproved = false } } label: { Text("Decline").font(.system(size: 14, weight: .semibold)).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 42).background(Color(hex: "8B1A1A")).clipShape(RoundedRectangle(cornerRadius: 10)) }
            }
        }
...
struct TransactionHistorySection: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Button { withAnimation(.spring(response: 0.35)) { expanded.toggle() } } label: {
                HStack(spacing: 12) {
                    ZStack { RoundedRectangle(cornerRadius: 10).fill(Color(hex: "EBF0F8")).frame(width: 38, height: 38); Image(systemName: "list.bullet.rectangle").font(.system(size: 16)).foregroundColor(Color(hex: "2D6DAB")) }
                    VStack(alignment: .leading, spacing: 2) {
